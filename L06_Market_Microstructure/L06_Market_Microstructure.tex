...
\vspace{0.3em}
Adverse selection: informed traders profit at uninformed traders' expense, forcing market makers to widen spreads.

\vspace{0.2em}
Kyle (1985) model:
\begin{itemize}
\item Price impact: $\lambda = \frac{\sigma_v}{2\sigma_u}$ (higher $\lambda$ = bigger price move per trade)
\item Informed trading intensity: the more insider information exists, the worse the price impact for everyone
\end{itemize}

\textbf{Example:} For Bitcoin, if $\sigma_v = 0.015$ (high information asymmetry) and $\sigma_u = 0.10$ (moderate noise trading), then $\lambda = 0.015/(2 \times 0.10) = 0.075$. For S\&P 500, $\lambda = 0.005/(2 \times 0.20) = 0.0125$---meaning Bitcoin trades move prices 6$\times$ more per unit traded.

\vspace{0.3em}
Glosten-Milgrom (1985):
\begin{itemize}
\item Bid-ask spread reflects asymmetry
\item Sequential trade learning (market makers update beliefs about asset value after each trade)
\end{itemize}
\end{columns}
...
\item Capital inefficiency (much of the deposited money sits idle at extreme prices)
...
IL formula: $IL = \frac{2\sqrt{r}}{1+r} - 1$ where $r = P_{\text{final}}/P_{\text{initial}}$.

\textbf{Example:} If you provide ETH/USDC liquidity and ETH doubles in price ($r=2$), $IL = \frac{2\sqrt{2}}{3} - 1 = -5.7\%$ vs just holding ETH and USDC.
...
\bottomnote{Sandwich attacks optimize profit $\pi = Q_f(P'-P) - 2\cdot\text{gas}$; welfare panels show transfers from users to extractors}
\end{frame}
...
\textbf{Actors}
\begin{itemize}
\item Searchers (bots scanning the mempool for profitable transaction orderings): find MEV opportunities
\item Builders (entities assembling transactions into candidate blocks): construct blocks
\item Validators (nodes that propose and finalize blocks on the network): propose blocks
\item Users: suffer MEV extraction
\end{itemize}

\vspace{0.2em}
{\small \textbf{Sandwich Profit:} $\pi = Q_f \cdot (P' - P) - 2 \cdot \text{gas}$

\textbf{Example:} Front-run $Q_f = 10$ ETH, price $P{=}\$1800 \to P'{=}\$1805$. Profit $= 10 \times 5 - 2 \times \$3 = \$44$.}
...
\item Flashbots auction mechanism (a private marketplace where searchers bid for block inclusion without spamming the network)
\item MEV-Boost infrastructure (middleware connecting validators to block builders for fair MEV distribution)
\end{itemize}
...
\begin{itemize}
\item Encrypted mempools (hiding pending transactions; the mempool is the waiting area for unconfirmed transactions)
\item Fair ordering protocols
\item Batch auctions
\item Threshold encryption (multiple parties must cooperate to decrypt)
\end{itemize}

\vspace{0.3em}
\textbf{Application-Level}
\begin{itemize}
\item MEV-protected RPCs (private endpoints for submitting transactions invisible to attackers)
\item Slippage tolerance limits
\item Time-weighted average pricing (splitting a large trade over time to reduce impact)
\item Private order flow (routing trades through channels hidden from MEV searchers)
\end{itemize}
\end{columns}
...
\item ZK-rollup order books (order books on blockchain scaling technology)
\end{itemize}
