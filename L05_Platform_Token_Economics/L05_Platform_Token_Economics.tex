...
\item Exhibits cross-side externalities (effects on third parties not involved in a transaction)
...
\textit{(The value for user $i$ is a function of total users $n$, and this value increases as $n$ grows)}
\end{itemize}
...
2020: 1M addresses $\rightarrow$ 200 dApps

2023: 2M addresses $\rightarrow$ 3000+ dApps
...
\begin{itemize}
\item ICOs (Initial Coin Offerings---token fundraising) reduce bootstrapping costs
...
\textit{(Token price increases with network activity but decreases with supply and velocity)}
...
\vspace{0.3em}
\textbf{Real Example: Token Velocity}

Token A: V=50/year (pure payment)

Token B: V=5/year (staking rewards)

With equal $M$ and $PQ$, Token B is worth 10x more.
\end{columns}
...
\item Example: Stablecoins (DAI (a decentralized stablecoin), Terra)
...
\item Payment networks: merchants and consumers
\item DeFi (Decentralized Finance) protocols: borrowers and lenders
\end{itemize}
...
\item Quadratic voting (voting power increases with square root of tokens, not linearly) (Weyl \& Lalley)
