...
# v(n) = v_0 + θ*n (intrinsic value + network value)
...
def actual_adoption(n_expected, N_max):
    ...
    Key: Stable equilibria occur where response crosses 45° line from ABOVE (slope < 1)
    ...
    # - Above 45° line for x < 0.15 (so it crosses down to create stable point)
    # - Below 45° line for 0.15 < x < 0.50 (below the line)
    # - Above 45° line for 0.50 < x < 0.90 (so it crosses down at 0.50 unstable, up at 0.90)
    # - Below 45° line for x > 0.90 (so it crosses down to create stable point)

    # Invert the cubic: use NEGATIVE amplitude
    # This makes the response curve go above-below-above-below relative to 45° line
...
ax.plot(n_e, n_actual, linewidth=3, label='Response Function', color=MLBLUE)
...
# Add coordination problem annotation
ax.text(0.05, 0.95, 'Coordination Problem:\n"Penguin Effect" - Nobody wants to be first',
        transform=ax.transAxes, fontsize=11, verticalalignment='top',
        bbox=dict(boxstyle='round,pad=0.6', facecolor='lightyellow',
                 edgecolor=MLORANGE, linewidth=2, alpha=0.9))

# Add subsidy effect annotation
ax.text(0.65, 0.25, 'Subsidy can escape\nlow equilibrium trap',
        transform=ax.transAxes, fontsize=10, verticalalignment='top',
        bbox=dict(boxstyle='round,pad=0.4', facecolor='lightgreen',
                 edgecolor=MLGREEN, linewidth=1.5, alpha=0.8))

ax.set_xlabel('Expected Adoption ($n^e$)')
ax.set_ylabel('Actual Adoption ($n$)')
ax.set_title('Platform Adoption: Katz-Shapiro Multiple Equilibria\n' +
             r'$u(n) = v_0 + \theta \cdot n^e - p$')
