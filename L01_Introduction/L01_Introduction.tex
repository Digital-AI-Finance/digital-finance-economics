...
\subtitle{L01: Setting the Economic Framework\\[0.3em]\normalsize Understanding the trillion-dollar shift from cash to code}
...
\item Apply four structured ways of thinking (economic frameworks) to digital finance
\end{itemize}
\end{columns}
...
\item Enable disintermediation (removing middlemen) or re-intermediation (replacing old middlemen with new digital ones like crypto exchanges)
\end{itemize}

\column{0.48\textwidth}
\textbf{Scope of Digital Finance}
\begin{itemize}
\item Cryptocurrencies (digital assets secured by cryptography---the mathematical science of encoding information to prevent tampering) and stablecoins (cryptocurrencies pegged to stable assets like USD)
...
% What is Digital Finance? (cont.)
\begin{frame}[t]{What is Digital Finance? (cont.)}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Key Economic Questions}
\begin{itemize}
\item Who captures value?
\item What are the welfare effects (overall impact on society's well-being)?
...
\begin{itemize}
\item Digital finance is not just a technology story---it rewires \textit{who} earns, \textit{who} pays, and \textit{who} bears risk
\item Every topic in this course returns to these three questions
\end{itemize}
\end{columns}

\bottomnote{This course focuses on economic incentives and outcomes, not on coding or protocols}
\end{frame}

% Historical Evolution
\begin{frame}[t]{Payment Technology Adoption Over Time}
\begin{center}
\includegraphics[width=0.55\textwidth]{01_payment_evolution/chart.pdf}
\end{center}

\begin{itemize}
\item Each S-curve shows how a payment technology starts slow, accelerates, then levels off
\item Notice how newer technologies (cards, digital) adopt \textit{faster} than older ones (checks)
\item The inflection point (steepest part of curve) marks the moment adoption accelerates rapidly
\end{itemize}

\bottomnote{Each transition was driven by economic forces: reducing transaction costs (time, fees, and difficulty in exchanges), enabling trade at scale}
\end{frame}
...
\item Enables intertemporal trade (saving now to spend later---trading value across time)
...
\item Stablecoins: Better but trust issues (are reserves really there? Who audits them?)
...
\begin{center}
\includegraphics[width=0.48\textwidth]{02_digital_finance_taxonomy/chart.pdf}
\end{center}

\begin{itemize}
\item This course examines digital finance from four complementary perspectives (lenses)
\item Do not worry about unfamiliar terms (AMMs, DEXs, etc.)---each lens is explained in detail on the next slides
\item Every digital finance topic can be analyzed through multiple lenses simultaneously
\end{itemize}

\bottomnote{This course applies all four lenses to understand digital finance comprehensively}
\end{frame}

% Four Economic Lenses: Comparative Relevance
\begin{frame}[t]{How the Four Lenses Apply to Each Topic}
\begin{center}
\includegraphics[width=0.55\textwidth]{04_four_economic_lenses/chart.pdf}
\end{center}

\begin{itemize}
\item Each spoke represents one of the four economic perspectives; further from center = more relevant
\item Bitcoin scores highly on Monetary and Regulatory lenses but lower on Platform Economics
\item DeFi protocols engage all four lenses roughly equally---they are the most multi-dimensional topic
\end{itemize}

\bottomnote{Radar chart: scores are illustrative relevance ratings (0--10) to help prioritize which lens to apply first for each topic}
\end{frame}
...
\item What happens to monetary policy transmission (how central bank decisions affect the real economy)?
...
\item Stablecoin stability mechanisms (how stablecoins maintain their peg through reserves, arbitrage, or algorithms)
\item Dollarization (adopting USD) vs.\ crypto-ization (adopting crypto instead of local currency)
\end{itemize}
\end{columns}
...
\item Two-sided market theory (how platforms like Visa connect merchants and cardholders, creating value for both sides)
...
\item Winner-take-all dynamics (markets where one player captures nearly all users, like Google in search)
...
\item Why are spreads (gaps between buy and sell prices) wider in crypto?
\item How do Automated Market Makers (AMMs) differ from order books (lists of pending buy and sell orders)?
\end{itemize}
...
\textbf{Key Questions}
\begin{itemize}
\item What market failures (situations where free markets produce harmful outcomes) justify regulation?
\item How should crypto be classified legally?
\item What are costs of regulatory arbitrage (exploiting differences in rules across jurisdictions)?
\end{itemize}
...
\item Regulators need welfare frameworks (tools for measuring impact on society's well-being)
...
\item Programmable financial contracts (agreements that automatically execute when conditions are met)
\end{itemize}

\vspace{0.3em}
\textbf{Research Opportunities}
\begin{itemize}
\item High-frequency blockchain data (transaction-level records updated every few seconds)
\item Natural experiments (real-world policy changes that create before/after comparisons) in adoption
...
\begin{center}
\includegraphics[width=0.52\textwidth]{03_global_digital_payments/chart.pdf}
\end{center}

\begin{itemize}
\item The shaded bands show uncertainty: the model runs many random scenarios (Monte Carlo simulation) to estimate a range of possible futures
\item CAGR (Compound Annual Growth Rate) measures average yearly growth---e.g., 15\% CAGR means the market grows roughly 15\% each year, compounding
\item Key takeaway: digital payments are growing rapidly regardless of model assumptions
\end{itemize}

\bottomnote{COVID-19 accelerated digital payment adoption; economists must understand these trends}
\end{frame}

% Digital Finance Growth Decomposition
\begin{frame}[t]{What Drives Digital Finance Growth?}
\begin{center}
\includegraphics[width=0.55\textwidth]{05_digital_finance_growth_decomposition/chart.pdf}
\end{center}

\begin{itemize}
\item Growth is decomposed (broken into parts) to show which factors contribute most: technology, adoption, network effects, or regulation
\item Early growth was technology-driven (smartphones, cloud); recent growth is adoption- and network-driven
\item Data is illustrative (not measured)---the pattern shows how economists think about growth accounting
\end{itemize}

\bottomnote{Growth accounting: total growth = technology contribution + adoption contribution + network effects + other factors}
\end{frame}
...
All exchange costs: time, fees, search, and difficulty.
