...
\subtitle{L08: Why analyzing digital finance requires combining all four economic lenses simultaneously}
...
\item TradFi-DeFi convergence (traditional and decentralized finance growing more similar)
\item Integrating the lenses for holistic analysis
\item The digital finance trilemma (impossible to maximize decentralization, security, and scalability simultaneously)
...
\item Token economics and mechanism design (designing rules so self-interested participants produce desired outcomes)
...
\bottomnote{Acemoglu et al.\ (2015) network contagion model: nodes fail when cumulative losses from neighbors exceed capital buffers; contagion severity depends on network topology. Allen \& Gale (2000) originated the framework; digital finance adds instant velocity}
\end{frame}

% Understanding Systemic Risk (Part 1)
...
\item Fire sales (forced selling at distressed prices) and liquidity spirals (price drops trigger more selling, pushing prices further down)
...
\bottomnote{Classic contagion models focus on network topology and capital buffers; see chart on previous slide}
\end{frame}

% Understanding Systemic Risk (Part 2)
\begin{frame}[t]{Understanding Systemic Risk Channels (cont.)}
\begin{columns}[T]
\column{0.48\textwidth}
...
\begin{itemize}
\item Oracle failures (oracles feed real-world data to smart contracts; wrong data triggers wrong decisions)
\item Protocol exploit contagion
\item Collateral liquidation cascades (borrowers fail to maintain collateral, assets are auto-sold, prices drop, triggering more liquidations)
\item Wrapped asset depegging (a wrapped asset is a token representing an asset from another blockchain; depegging means losing its 1:1 value linkage)
\end{itemize}
\end{columns}
...
\item Payment rails (underlying infrastructure for transferring money) integration
...
\begin{itemize}
\item Permissioned DeFi (DeFi protocols with restricted participation, combining decentralized technology with traditional compliance)
\item Tokenized deposits (bank deposits represented as digital tokens on a blockchain, enabling programmable transfers)
...
\textit{Example: USDC depeg (March 2023)---exposure to Silicon Valley Bank failure showed TradFi-DeFi interconnection. JPMorgan Onyx shows institutional blockchain adoption (tokenized deposits, programmable payments).}
\end{columns}

\bottomnote{Each case study requires all four lenses for complete analysis---single-lens thinking misses critical interactions}
\end{frame}

% Integration Examples (Part 2)
\begin{frame}[t]{Case Studies: Integrating Multiple Lenses (cont.)}
\begin{columns}[T]
\column{0.48\textwidth}
...
\textit{Example: Uniswap vs.\ SushiSwap---competing DEX protocols illustrate platform forks, liquidity migration, and governance token incentives.}
...
\vspace{0.3em}
\textbf{Cross-Lens Interactions}

Every real-world event involves \textit{all four lenses}. The skill is identifying which lens dominates and how they interact.
\end{columns}
...
\bottomnote{Similar to Mundell's impossible trinity (can't have fixed exchange rate, free capital flow, AND independent monetary policy), digital finance faces fundamental trade-offs between decentralization, security, and scalability}
\end{frame}
...
\textbf{2. Security}
\begin{itemize}
\item Resistance to attacks and fraud
...
\textbf{3. Scalability}
\begin{itemize}
\item High throughput (transactions per second), low latency
\item Low transaction costs
\item Capacity to serve millions of users
\end{itemize}
...
\vspace{0.3em}
\textbf{Decentralized + Scalable}
\begin{itemize}
\item Example: Solana, early DeFi protocols
\item Problem: Harder to secure against attacks
\end{itemize}

\vspace{0.3em}
\textbf{Decentralized + Secure}
\begin{itemize}
\item Example: Bitcoin, Ethereum (proof-of-work)
\item Problem: Slow, expensive (scalability loss)
\end{itemize}

\vspace{0.3em}
\textbf{Secure + Scalable}
\begin{itemize}
\item Example: Visa, CBDCs, permissioned blockchains
\item Problem: Centralized control (decentralization loss)
\end{itemize}

\vspace{0.3em}
\textbf{Formal Constraint:} $D + S + Sc \leq K$

\smallskip
\textbf{Example:} If $K = 1$, Bitcoin allocates $D{=}0.45$, $S{=}0.45$, $Sc{=}0.10$. Visa allocates $D{=}0.05$, $S{=}0.45$, $Sc{=}0.50$. Both sum to $\leq 1$, but with very different trade-offs.
\end{columns}

\bottomnote{Constraint: D + S + Sc $\leq$ K. Fundamental trade-offs constrain design space; technological advances may shift boundaries but not eliminate the trilemma}
\end{frame}
...
\bottomnote{Chart shows five policy instruments (CBDC, Stablecoin Regulation, DeFi Rules, Crypto Tax, KYC/AML) scored against four objectives (Financial Inclusion, Stability, Innovation, Consumer Protection). Green = positive effect, red = negative effect}
\end{frame}
...
\item Lending platforms: DeFi vs. CeFi (Centralized Finance---platforms that custody user funds on internal systems) ambiguity
\end{itemize}
...
\textit{These are open research questions at the frontier of digital finance. The four economic lenses you learned in this course provide the foundation for engaging with them.}
...
\begin{itemize}
\item Agent-based simulation (computer models where many individual actors interact, revealing emergent system-wide behavior)
...
Trade-off between decentralization, security, and scalability---difficult to maximize all three simultaneously (constraint: $D + S + Sc \leq K$).
