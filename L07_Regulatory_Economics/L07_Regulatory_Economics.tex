...
\subtitle{L07: Costs, Benefits, and Arbitrage\\[0.3em]\small Why do some crypto exchanges operate legally while others collapse? Regulatory economics explains when and how rules shape digital finance.}
...
% Why Regulate Digital Finance? -- Market Failures
\begin{frame}[t]{Why Regulate Digital Finance? Market Failures}
\begin{columns}[T]
\column{0.48\textwidth}
...
\begin{itemize}
\item Complex technical protocols (sets of rules governing data exchange between systems)
\item Opaque risk disclosures
\item Retail investor (individual, non-professional investor) sophistication gaps
\end{itemize}

\vspace{0.3em}
\textbf{2. Externalities (costs/benefits affecting uninvolved parties)}
\begin{itemize}
\item Systemic risk contagion (cascading failures spreading across interconnected institutions)
...
\textit{Natural monopoly}: a market where one firm can supply at lower cost than multiple competitors.
\begin{itemize}
\item Network effects in payment systems
\item Infrastructure control (control over essential shared systems like payment networks)
\item Data monopolies (dominance from accumulating exclusive user transaction data)
\end{itemize}
...
\item Irreversible transactions (once confirmed on the blockchain, transactions cannot be undone)
\item Custody and loss risks
\end{itemize}
\end{columns}

\bottomnote{These four market failures are the economic justification for regulation---without them, free markets would suffice}
\end{frame}

% Why Regulate Digital Finance? -- Public Interest
\begin{frame}[t]{Why Regulate Digital Finance? Public Interest Rationale}
\begin{columns}[T]
\column{0.48\textwidth}
\textbf{Beyond Market Failures}

Even when markets function, regulation serves broader public goals:

\vspace{0.3em}
\textbf{Public Interest Goals}
\begin{itemize}
\item \textbf{Financial stability}---preventing crises that harm the entire economy
\item \textbf{Consumer protection}---shielding individuals who cannot protect themselves
\item \textbf{Market integrity}---ensuring prices reflect genuine supply and demand, not manipulation
\end{itemize}

\column{0.48\textwidth}
\textbf{But Who Does Regulation Really Serve?}

\vspace{0.3em}
\textbf{Stigler's Regulatory Capture Theory}

Stigler (1971) warned that regulation can be ``captured'' by the very industry it oversees:
\begin{itemize}
\item Regulated firms lobby for rules that benefit them
\item Barriers to entry protect incumbents from competition
\item Regulators may prioritize industry relationships over public welfare
\end{itemize}

\textit{Example: Large banks lobbied for complex Basel rules that small banks cannot afford to implement---reducing competition.}
\end{columns}

\bottomnote{Stigler (1971): Regulation can serve public interest or private capture (regulatory capture)---evidence determines which}
\end{frame}
...
\smallskip\textbf{Example:} A new KYC rule costs exchanges \$30M/year in compliance but prevents \$50M/year in fraud losses. Since marginal benefit (\$50M) $>$ marginal cost (\$30M), this rule is worth adding. A \textit{second} rule costs another \$40M but only prevents \$20M more fraud---this one fails the test and should not be imposed.
...
\textit{(``Lemons'' is American slang for defective cars---Akerlof showed how information asymmetry degrades entire markets)}

Without regulation:
\begin{itemize}
\item Good projects cannot signal quality
\item Bad projects dominate (adverse selection: a market breakdown where buyers, unable to distinguish quality, offer average prices---causing high-quality sellers to exit, leaving only low-quality ``lemons'')
...
Welfare means total economic value to society, combining:
\begin{itemize}
\item \textbf{Consumer surplus} (the extra value buyers get above what they actually pay---e.g., you'd pay \$5 for coffee but it costs \$3, so your surplus is \$2)
\item \textbf{Producer surplus} (profit sellers earn above their minimum acceptable price)
\end{itemize}

\begin{itemize}
\item Regulation can destroy surplus: foregone efficient transactions
\item Optimal regulation minimizes total welfare loss
\end{itemize}
...
\vspace{0.3em}
\textbf{Regulatory Perimeter}: The boundary between regulated and unregulated activities. As the chart shows, when regulation intensity increases: \textbf{compliant firms} decrease (green), some move to the \textbf{informal sector} (orange, operating without licenses), and others \textbf{exit or relocate} (red, moving to jurisdictions with lighter rules). The policy challenge: set intensity where compliance is high but not so harsh that firms flee.

\bottomnote{Chart: stacked area shows firm responses to regulation intensity---compliant, informal, or relocated}
\end{frame}
...
\item Are NFTs (Non-Fungible Tokens---unique, not interchangeable; each represents a distinct digital item) securities or commodities?
\item Are DAOs (Decentralized Autonomous Organizations---governed by code rules voted on by token holders, not by traditional managers) legal entities?
\item Are validators (network participants who verify and confirm transactions on a blockchain) financial intermediaries?
\end{itemize}
...
\vspace{0.3em}
\textbf{Key term:} MES (Minimum Efficient Scale)---the smallest firm size at which compliance cost per dollar of revenue is minimized. Firms smaller than MES face disproportionately high compliance burdens.

\textbf{Note the U-shape:} Beyond MES, costs rise again---very large firms face \textit{diseconomies} of scale (increased regulatory scrutiny, complexity of multi-jurisdiction compliance, and systemic-importance surcharges).

\bottomnote{Stigler (1971): Regulation favors large firms through economies of scale---but the chart's U-shape shows even giants face rising costs at extreme scale}
\end{frame}
...
\item Technology infrastructure (shared technical systems---servers, networks, and software---on which financial services run)
...
\item Regulatory sandboxes (controlled experimentation zones---discussed in detail later this lesson)
...
\column{0.48\textwidth}
\textbf{3. Temporal Arbitrage} (exploiting the time gap between a new financial activity appearing and regulators creating rules for it)
\begin{itemize}
...
\item Basel accords (international agreements setting minimum capital requirements for banks) for capital
...
\begin{itemize}
\item Long-arm jurisdiction (authority to regulate foreign companies that serve domestic customers)
...
\bottomnote{Akerlof (1970): Disclosure requirements mitigate the lemons problem by reducing information asymmetry}
\end{frame}
...
\item Prohibited practices (pump-and-dump: artificially inflating a token's price through false hype, then selling at the peak)
\item Custody standards (self-custody: holding own private keys)
\item Fiduciary duties (legal obligation to act in the client's best interest, not your own)
\end{itemize}
...
\vspace{0.3em}
\textbf{Bounded Rationality} (people make decisions with limited information, time, and cognitive capacity---they cannot perfectly optimize)
\begin{itemize}
...
\vspace{0.3em}
\textbf{Proportionality Principle} (regulatory burden should match the level of risk posed)
\begin{itemize}
...
\begin{itemize}
\item Soft law (non-binding guidelines that influence behavior without legal force) and principles
...
\vspace{0.3em}
\textbf{Embedded Supervision} (regulators read blockchain data directly to verify compliance, replacing manual reporting)
\begin{itemize}
\item Real-time compliance monitoring
\item Automated reporting (RegTech)
\item Smart contract-based rules (self-executing programs on a blockchain that automatically enforce agreed terms)
...
\textbf{Looking Ahead}
\begin{itemize}
\item L08: Synthesis of all four lenses (monetary economics, payment systems, platform/token economics, and regulatory economics)
...
% Key Terms
\begin{frame}[t]{Key Terms (1/3) --- Reference Glossary}
\textit{These three slides serve as a reference glossary. You do not need to memorize every term---use them to look up definitions when studying.}
\vspace{0.3em}
\begin{columns}[T]
\column{0.48\textwidth}
