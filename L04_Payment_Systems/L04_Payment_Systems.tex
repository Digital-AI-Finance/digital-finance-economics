...
\textit{(Below 16\%: must convince each user individually. Above 16\%: ``everyone uses it''---adoption accelerates)}
...
\item Anti-competitive coordination (competitors secretly agreeing on prices instead of competing)
\end{itemize}
...
\bottomnote{EU capped interchange at 0.2-0.3\%; US Durbin Amendment capped debit at 0.05\%{}+21c. EU caps are lower due to stronger regulator intervention.}
\end{frame}
...
\begin{itemize}
\item Expensive: 6\%{}+ average cost (send \$100, only \$94 arrives)
...
\textit{(Example: A owes B \$5M, B owes A \$4M. DNS: settle net \$1M. RTGS: two separate \$9M total transfers)}
\end{itemize}
...
\item US: FedNow (2023)
\textit{(US was late: thousands of fragmented banks made coordination hard; existing systems were ``good enough'' for incumbents)}
\end{itemize}
...
\item DeFi (Decentralized Finance---financial services on blockchain without traditional banks) payment rails
\end{itemize}
...
\item Open banking mandates (regulations requiring banks to share customer data with authorized apps)
